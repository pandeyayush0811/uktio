...
// FIRST-CHUNK variant of SENTENCE_RE: additionally breaks on a comma.
// Used ONLY for sentenceIndex === 0 (see the loop below) — the whole
// point is to get Sarvam SOMETHING small to start synthesizing on as
// early as possible, since TTS time (not LLM generation time) is the
// dominant part of the text-to-audio gap. A full first sentence can
// easily be 60-100+ chars before its '.'; breaking at the first comma
// instead (when there is one) can hand Sarvam a synthesizable chunk
// several hundred milliseconds sooner. Every sentence AFTER the first
// still uses the normal SENTENCE_RE — this only shaves the very first,
// most latency-sensitive chunk of a turn.
const FIRST_CHUNK_RE = /^[^.!?।॥,]*[.!?।॥,]+(\s+|$)/;
...
async function streamReply(history, userText, onSentence) {
  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      let match;
      while (sentenceIndex < MAX_SENTENCES_PER_REPLY && (match = buffer.match(sentenceIndex === 0 ? FIRST_CHUNK_RE : SENTENCE_RE))) {
