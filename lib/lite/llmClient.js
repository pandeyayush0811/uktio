...
async function streamReply(history, userText, onSentence) {
  ...
  // just literal trailing text), then flush a trailing partial sentence
  // (no ./!/? yet) — only if the stream ended naturally. If we hit the
  // cap mid-sentence, better to end clean on the last complete sentence
  // than play/show a sentence truncated mid-word.
  const strippedTail = tagStripper.flush();
  if (strippedTail && !hitCap) {
    buffer += strippedTail;
    full += strippedTail;
  }
  const leftover = buffer.trim();
  if (leftover && !hitCap && sentenceIndex < MAX_SENTENCES_PER_REPLY) {
